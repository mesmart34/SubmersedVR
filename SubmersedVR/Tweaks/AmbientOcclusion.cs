using System.IO;
using SubmersedVR.AmplifyOcclusion.Plugins;
using SubmersedVR.VR;
using UnityEngine;

namespace SubmersedVR.Tweaks;

internal static class AmbientOcclusionVR
{
    ...
    public static void Init()
    {
        ...
        ShaderLoader.Initialize(Path.Combine(Application.streamingAssetsPath, AMPLIFY_SHADERS));
        initialized = true;
    }
    ...
    public static void OnAmbientOcclusionSettingsChanged(Camera camera)
    {
        ...
        if (effect != null)
        {
            ...
            effect.SampleCount = Settings.AOMethod == "Low" ? SampleCountLevel.Low : (Settings.AOMethod == "High" ? SampleCountLevel.High : (Settings.AOMethod == "Very High" ? SampleCountLevel.VeryHigh : SampleCountLevel.Medium));
