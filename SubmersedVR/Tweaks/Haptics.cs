...
public static class HapticsVR
{
    ...
    public static void PlayHapticsNew(Controller controller, float secondsFromNow, float durationSeconds, float frequency, float amplitude)
    {
        switch (controller)
        {
            case Controller.Left:
                SteamVR_Actions.subnautica_HapticsLeft?.Execute(secondsFromNow, durationSeconds, frequency, amplitude, SteamVRRef::Valve.VR.SteamVR_Input_Sources.Any);
                break;
            case Controller.Right:
                SteamVR_Actions.subnautica_HapticsRight?.Execute(secondsFromNow, durationSeconds, frequency, amplitude, SteamVRRef::Valve.VR.SteamVR_Input_Sources.Any);
                break;
    ...
    {
        var controller = Settings.PutBarsOnWrist ? Controller.Left : Controller.Both;
...
public static class ExosuitDrillHaptics
{
    public static void Postfix(Exosuit __instance)
    {
        var left = __instance.leftArm as ExosuitDrillArm;
        var right = __instance.rightArm as ExosuitDrillArm;
        if ((left != null && left.drilling) || (right != null && right.drilling))
        {
            ...
            {
                var dur = UnityEngine.Random.Range(0.01f, 0.20f);
                if (right?.drilling == true)
                {
                    ...
                    var amp = UnityEngine.Random.Range(0f, 1.0f) * (right?.drillTarget == true ? 1.0f : 0.1f);
                ...
                if (left?.drilling == true)
                {
                    ...
                    var amp = UnityEngine.Random.Range(0f, 1.0f) * (left?.drillTarget == true ? 1.0f : 0.1f);
...
public static class ExosuitClawArmHaptics
{
    ...
    {
        var controller = __instance.exosuit.rightArm as ExosuitClawArm == __instance ? HapticsVR.Controller.Right : HapticsVR.Controller.Left;
...
public static class ExosuitGrapplingActiveArmHaptics
{
    public static void Postfix(ExosuitGrapplingArm __instance)
    {
        var isRightarm = __instance.exosuit.rightArm as ExosuitGrapplingArm == __instance;
        var isLeftArm = __instance.exosuit.leftArm as ExosuitGrapplingArm == __instance;
...
public static class DamageOverTimeHaptics
{
    public static void Postfix(DamageOverTime __instance)
    {
        ...
        var dur = __instance.interval - 0.1f;
        if (dur < 0)
        {
            dur = 0.1f;
        }
...
public static class PlayerDamageHaptics
{
    ...
    {
        var intensity = damageInfo.damage * 3f / 100f;
        var shakeAmount = Mathf.Clamp(intensity, 0f, 5f);
...
class PropulsionCannonShootHaptics
{
    public static void Postfix(PropulsionCannon __instance)
    {
        ...
        // }
        var rightArm = true;
        var exosuit = VehiclesVR.PilotedExosuit();
        if (exosuit != null)
        {
            rightArm = (exosuit.rightArm as ExosuitPropulsionArm)?.propulsionCannon == __instance;
        }
        ...
        var controller = rightArm ? HapticsVR.Controller.Right : HapticsVR.Controller.Left;
    ...
    public static void Postfix(PropulsionCannon __instance)
    {
        if ((__instance.grabbedObject != null && __instance.grabbedObject.GetComponent<Rigidbody>() != null) || __instance.firstUseGrabbedObject != null)
        {
            ...
            else
            {
                ...
                {
                    var controller = __instance.firstUseGrabbedObject != null ? HapticsVR.Controller.Both : HapticsVR.Controller.Right;
...
class StasisRifleChargeHaptics
{
    public static void Postfix(StasisRifle __instance)
    {
        if (__instance.isCharging) {
            ...
            var charge = __instance.chargeAmount;
            var freq = charge * 10.0f;
            var amp = charge * 1.0f/10.0f; // 0.0 to 0.5
            if (charge >= 4.9f) {
                amp = 0.0f;
            }
